<template>
    <div class="rounded border border-gray-600 shadow-lg md:border-0 mt-3 mx-3 flex flex-wrap col-span-12 lg:col-span-6"
         v-bind:class="{'xl:col-span-12': this.size === 1, 'xl:col-span-6': this.size === 2, 'xl:col-span-4': this.size === 3}">
        <div class="px-2 pt-6 text-center w-full self-start">
            <div v-for="category in item.categories" class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mb-1 w-1/2 truncate">{{ category.name }}</div>
        </div>
